...
class SimpleLogMiddleware(LoggingMiddleware):
	...
	def process_response(self, request, response, data):
		...
		resp_log = "Request on {} {} ended with code: {}".format(request.method, request.get_full_path(), response.status_code)
		...
		try:
			self.logger.log(self.log_level, f'User: {request.user.id}, Account: {request.user.account.id or None}', logging_context)
		except Exception:
